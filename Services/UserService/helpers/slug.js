// Based on https://github.com/talha-asad/mongoose-url-slugs
...
module.exports = (slugFields, options) => {
    options = {
        ...defaultOptions,
        ...options,
    };
    ...
    return (schema => {
        ...
        schema.methods.ensureUniqueSlug = function (slug, cb) {
            if (!options.indexUnique) {
                return cb(null, slug);
            }

            const doc = this;
            const model = doc.constructor;
            const slugLimited = (options.maxLength && slug.length === options.maxLength);
            const q = {};
            const fields = {};

            q._id = {$ne: doc._id};
            q[options.field] = new RegExp('^' + (slugLimited ? slug.slice(0, slug.length - 2) : slug));
            fields[options.field] = 1;
            model.find(q, fields).exec((e, docs) => {
                if (e) {
                    return cb(e);
                } else if (!docs.length) {
                    return cb(null, slug);
                } else {
                    let max = docs.reduce((max, doc) => {
                        const docSlug = doc.get(options.field, String);
                        let count = 1;
                        if (docSlug !== slug) {
                            count = docSlug.match(new RegExp((slugLimited ? slug.slice(0, slug.length - 2) : slug) + options.separator + '([0-9]+)$'));
                            count = (count instanceof Array) ? parseInt(count[1]) + 1 : 0;
                        }
                        return count > max ? count : max;
                    }, 0);

                    if (max === 1) { // avoid slug-1, rather do slug-2
                        max++;
                    }

                    const suffix = max === 0 ? '' : options.separator + max;

                    if (options.maxLength) {
                        return cb(null, slug.slice(0, options.maxLength - suffix.length) + suffix);
                    } else {
                        return cb(null, slug + suffix);
                    }
                }
            });
        };
        ...
        schema.pre(options.onHook, function (next) {
            const doc = this;
            const currentSlug = doc.get(options.field, String);
            let slugFieldsModified = doc.isNew;
            let slugFieldsSelected = true;

            if (!doc.isNew && !options.update && currentSlug) {
                // Skip if it's an edit and the plugin is configured to not update.
                return next();
            } else if (!doc.isNew && doc.isModified(options.field) && currentSlug && !options.alwaysRecreate) {
            ...
            for (let i = 0; i < slugFields.length; i++) {
                const slugField = slugFields[i];
                if (!doc.isSelected(slugField)) {
                    slugFieldsSelected = false;
                }
                if (doc.isModified(slugField)) {
                    slugFieldsModified = true;
                }
                const slugPart = doc.get(slugField, String);
                if (slugPart !== undefined) {
                    toSlugify += slugPart + ' ';
                }
            }
            if (toSlugify.length) {
                toSlugify = toSlugify.slice(0, toSlugify.length - 1);
            } else {
                toSlugify = options.undefinedVal;
            }

            // Skip setting slug if slug dependant fields were not selected
            if (!slugFieldsSelected) {
                return next();
            }

            if (!options.alwaysRecreate && !slugFieldsModified && currentSlug) {
                return next();
            }
            ...
            if (options.indexSparse && newSlug === options.undefinedVal) {
                ...
                doc.markModified(options.field);
                return next();
            }

            if (options.maxLength) {
                newSlug = newSlug.slice(0, options.maxLength);
            }

            doc.ensureUniqueSlug(newSlug, (e, finalSlug) => {
                if (e) {
                    return next(e);
                }
